function fibe() {
  ...
  var decor = undefined;
  var style = undefined;
  var button1_name = undefined;
  var button1_color = undefined;
  var button1_action = undefined;
  var button2_name = undefined;
  var button2_color = undefined;
  var button2_action = undefined;
  var type = "alert";
    ...
    if (title != undefined) {
      fibeRender(function() {
        ...
        if (type == "confirm") {
          ...
          $(".fibe-buttons-row").append(
            '<div class="fibe-button2"><span>' + button2_name + "</span></div>"
          );
...
function findStyle(myvar) {
  if (myvar.match(/^\#([a-f|0-9]){3,6}$/i)) {
    ...
    myvar.match(
      /^rgba\(\s?\d{1,3}\s?\,\s?\d{1,3}\s?\,\s?\d{1,3}\s?\,\s?\d?\.?\d{1,4}?\s?\)$/i
    )
  ) {
    return true;
  } else if (
    myvar.match(/^rgb\(\s?\d{1,3}\s?\,\s?\d{1,3}\s?\,\s?\d{1,3}\s?\)$/i)
  ) {
    return true;
...
function fibeRender(callback) {
  ...
  $(".fibe-buttons-row").append(
    '<div class="fibe-button1"><span>Ок</span></div>'
  );
...
function fibeSetStyle(style) {
  if (style == "success") {
    $(".fibe-window-decor, .fibe-button1, .fibe-button2").css(
      "background-color",
      "#72c34a"
    );
  } else if (style == "error") {
